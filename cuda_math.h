...
template<class T> inline __host__ __device__ T power(T s, const int factor) {
  if(factor==0) return 1; else
  if(factor==1) return s; else
  if(factor==2) return s*s; else
  if(factor==3) return s*s*s; else
  if(factor==4) return s*s*s*s; else
  if(factor==5) return s*s*s*s*s; else
  if(factor==6) return s*s*s*s*s*s; else
  if(factor==7) return s*s*s*s*s*s*s; else
  return pow(s, double(factor));
}
